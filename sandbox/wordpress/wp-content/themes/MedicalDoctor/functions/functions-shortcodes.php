...
class WPBakeryShortCode_madza_custom_post_type extends WPBakeryShortCode {
    ...
    protected function content($atts, $content = null) {
        ...
        global $post;
		$myposts = get_posts('post_type='. $posttype .'&order='. $sorting .'&posts_per_page='.$item_nr);
		$_home_portfolio = '';
		foreach($myposts as $post){
			...
			setup_postdata($post);
					    ...
					    if($posttype=="our-doctors") { 
						    ...
						    $education = '<p>'. substr($portfolio_caption, 0, $excerpt) .'...</p>';
						    
					    }
					    
					 
					     
					     if($columns=="2") { $column = "span6"; }
					     if($columns=="3") { $column = "span4"; }
					     if($columns=="4") { $column = "span3"; }
					     if($columns=="") { $column = "span4"; }
					     
					     $css_class = "";
					     $css_class .= $this->getCSSAnimation($css_animation);
						 $_home_portfolio .= '<div class="'. $column .''.$css_class.'">
						  <figure>
...
return'<div class="row grid cs-style-3">'. $_home_portfolio .'</div>';
...
wpb_map( array(
    ...
    "params"    => array(
        array(
            ...
            "param_name" => "posttype",
            "value" => array('Portfolio' => 'portfolio', 'Post' => 'post', 'Causes' => 'causes', 'Doctors' => 'our-doctors', 'Services' => 'our-services'),
            "description" => __("Set Custom Post Type.", "js_composer")
        ),
        array(
            "type" => "dropdown",
...
function ul( $atts, $content = null ) {
   return '<ul class="ul-shortcode">' . do_shortcode($content) . '</ul>';
}
add_shortcode('ul', 'ul');
